...
本项目使用 Google Gemini API（gemini-3-flash-preview）进行截图分析与总结。根据实际使用测试，**截图+AI 总结服务运行时每小时约消耗 0.3 ~ 0.4 美元的 Gemini API 费用**。

费用与以下因素相关：
- 截图间隔（默认 5 秒）
- 图片尺寸和质量
- AI 总结粒度（2min / 10min / 1h）
- 屏幕内容变化频率（无变化时会自动跳过 API 调用）

建议在配置中设置合理的时间窗口限制和自动停止时间，避免非工作时间产生不必要的费用。

## 隐私风险提示

**请在使用前充分了解以下隐私风险：**

1. **屏幕截图上传**：本软件会按固定间隔（默认每 5 秒）截取你的完整屏幕画面，并将截图上传至 Google Gemini API 进行分析。这意味着你屏幕上显示的**所有内容**（包括但不限于聊天记录、邮件、密码输入框、个人文件、浏览器页面等）都可能被传输至 Google 服务器。

2. **焦点窗口信息**：软件会持续记录你当前使用的应用名称和窗口标题，这些信息同样会作为 prompt 上下文发送给 AI。

3. **数据留存**：截图文件和 AI 总结结果会保存在本地磁盘。请确保你的设备存储安全。

4. **使用建议**：
   - 在处理敏感信息时考虑暂停服务
   - 定期清理过期截图
   - 不要在共享或不受信任的设备上运行
   - 了解 [Google Gemini API 的数据使用政策](https://ai.google.dev/terms)
